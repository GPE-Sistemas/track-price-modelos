  ...
  if (typeof queryParams?.filter === 'object') {
    for (const filtro of queryParams.filter) {
      if (typeof filtro.field === 'string') {
        parsedQuery.filter[filtro.field] = getFilterValue(
          filtro.type,
          filtro.value
        );
      } else {
  ...
  try {
    ...
    if (/^2/.test('' + response.statusCode)) {
      ...
      throw {
        ...
        mensaje:
          parsedBody.mensaje || parsedBody.message || response.statusMessage,
      };
    }
  } catch (err: any) {
    switch (err.code) {
...
export async function compareClave(
  clave: string,
  hash: string
): Promise<boolean> {
