...
import {
  ...
  IOperadorDTO,
  IPagoRelevamientoDb,
  IPagoRelevamientoDTO,
  IPrincipioActivoDb,
  IPrincipioActivoDTO,
  IProductoDb,
  IProductoDTO,
  IProductoRelevamientoDb,
  IProductoRelevamientoDTO,
  IProvinciaDb,
  IProvinciaDTO,
  IRegionDb,
  IRegionDTO,
  ...
  ISubsegmentoDb,
  ISubsegmentoDTO,
  IUbicacionRelevamientoDb,
  IUbicacionRelevamientoDTO,
  IZonaDb,
  IZonaDTO,
  ISubsegmentoPropioDb,
...
export class TrackPriceParserService {
  ...
  static departamentos(
    datos: LeanDocument<IDepartamentoDb>[]
  ): IDepartamentoDTO[] {
    const dto: IDepartamentoDTO[] = [];
    for (const dato of datos) {
  ...
  static subsegmentos(
    datos: LeanDocument<ISubsegmentoDb>[]
  ): ISubsegmentoDTO[] {
    const dto: ISubsegmentoDTO[] = [];
    for (const dato of datos) {
  ...
  static subsegmentoPropio(
    dato: LeanDocument<ISubsegmentoPropioDb>
  ): ISubsegmentoPropioDTO {
  ...
  static subsegmentosPropios(
    datos: LeanDocument<ISubsegmentoPropioDb>[]
  ): ISubsegmentoPropioDTO[] {
    const dto: ISubsegmentoPropioDTO[] = [];
    for (const dato of datos) {
  ...
  static familiaQuimica(
    dato: LeanDocument<IFamiliaQuimicaDb>
  ): IFamiliaQuimicaDTO {
  ...
  static familiasQuimicas(
    datos: LeanDocument<IFamiliaQuimicaDb>[]
  ): IFamiliaQuimicaDTO[] {
    const dto: IFamiliaQuimicaDTO[] = [];
    for (const dato of datos) {
  ...
  static principioActivo(
    dato: LeanDocument<IPrincipioActivoDb>
  ): IPrincipioActivoDTO {
  ...
  static principiosActivos(
    datos: LeanDocument<IPrincipioActivoDb>[]
  ): IPrincipioActivoDTO[] {
    const dto: IPrincipioActivoDTO[] = [];
    for (const dato of datos) {
  ...
  static composiciones(
    datos: LeanDocument<IComposicionDb>[]
  ): IComposicionDTO[] {
    const dto: IComposicionDTO[] = [];
    for (const dato of datos) {
  ...
  static producto(dato: LeanDocument<IProductoDb>): IProductoDTO {
    const dto: IProductoDTO = {
      ...
      idsSubsegmentosPropios: dato.idsSubsegmentosPropios?.map((id) =>
        id.toHexString()
      ),
  ...
  static pagoRelevamiento(
    dato: LeanDocument<IPagoRelevamientoDb>
  ): IPagoRelevamientoDTO {
  ...
  static pagosRelevamiento(
    datos: LeanDocument<IPagoRelevamientoDb>[]
  ): IPagoRelevamientoDTO[] {
    const dto: IPagoRelevamientoDTO[] = [];
    for (const dato of datos) {
  ...
  static productoRelevamiento(
    dato: LeanDocument<IProductoRelevamientoDb>
  ): IProductoRelevamientoDTO {
  ...
  static productosRelevamiento(
    datos: LeanDocument<IProductoRelevamientoDb>[]
  ): IProductoRelevamientoDTO[] {
    const dto: IProductoRelevamientoDTO[] = [];
    for (const dato of datos) {
  ...
  static ubicacionRelevamiento(
    dato: LeanDocument<IUbicacionRelevamientoDb>
  ): IUbicacionRelevamientoDTO {
  ...
  static ubicacionesRelevamiento(
    datos: LeanDocument<IUbicacionRelevamientoDb>[]
  ): IUbicacionRelevamientoDTO[] {
    const dto: IUbicacionRelevamientoDTO[] = [];
    for (const dato of datos) {
  ...
  static relevamientos(
    datos: LeanDocument<IRelevamientoDb>[]
  ): IRelevamientoDTO[] {
    const dto: IRelevamientoDTO[] = [];
    for (const dato of datos) {
  ...
  // Grupos Comparativos
  static grupoComparativo(
    dato: LeanDocument<IGrupoComparativoDb>
  ): IGrupoComparativoDTO {
    const dto: IGrupoComparativoDTO = {
      _id: dato._id?.toHexString(),
      nombre: dato.nombre,
      fecha: dato.fecha?.toISOString(),
      idProductos: dato.idProductos?.map((id) => id.toHexString()),
  ...
  static gruposComparativos(
    datos: LeanDocument<IGrupoComparativoDb>[]
  ): IGrupoComparativoDTO[] {
    const dto: IGrupoComparativoDTO[] = [];
    for (const dato of datos) {
